...
mergemonthday <- function(){
        #Merge two xts objects, one monthly and one dayly, replies the month into
        #Dayly
...
Data_Index_Level_R   <-initialTransform(read.csv("~/Upwork/Betasmartz - Black Litterman/FED's Model/IndexDataR.csv",stringsAsFactors=FALSE))
...
#Extract the date and coerce the 
date_test<-extractDate(Data_Month_Perc_Trend[,24])
